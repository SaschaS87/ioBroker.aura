        ...
        tabs.map((tab, idx) => (
            <TabConditionWrapper key={tab.id} tab={tab}>
                {({ cssVars, bold, italic, effect, hidden }) => {
                    ...
                    return (
                        <div
                            className={`group relative flex items-center gap-1.5 px-2 cursor-pointer transition-colors whitespace-nowrap select-none ${indicatorStyle === 'underline' ? 'py-2.5 border-b-2' : 'py-1.5'} ${bold ? 'aura-cond-bold' : ''} ${italic ? 'aura-cond-italic' : ''} ${!editMode && effect === 'border' ? 'aura-cond-ring' : ''}`}
                            style={{
