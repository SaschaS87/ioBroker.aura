...
import { ShutterRoomDef, ShutterDeviceDef } from './types';
import { PendingContext, PendingState } from './useShutterDevice';
...
export const ShutterRoomsWidget: React.FC<WidgetProps> = ({ config }) => {
  ...
  // Fassaden-Filter
  const [activeFacade, setActiveFacade] = useState<string>(
    configFacades[0] || 'Alle'
  );

  // Sheet-State
  const [selectedDevice, setSelectedDevice] = useState<ShutterDeviceDef | null>(
    null
  );
    ...
    return rooms
      .map((room) => {
        const filteredDevices = room.devices.filter((dev) => {
          ...
          return (
            activeFacade === 'Alle' ||
            (effFacade && effFacade.includes(activeFacade))
          );
        });
        ...
        <div className="facades-bar">
          <div className="facades-scroll">
            {configFacades.map((facade) => (
              <button
                key={facade}
                className={`facade-chip ${
                  activeFacade === facade ? 'active' : ''
                }`}
        ...
        <div className="rooms-scroll">
          {filteredRooms.length === 0 ? (
            <div className="empty-state">
              Keine Geräte für {activeFacade} sichtbar
            </div>
          ) : (
            filteredRooms.map((room) => (
              <section key={room.name} className="room-section">
                <div className="room-header">
                  ...
                  {room.facade && (
                    <span className="room-facade">{room.facade}</span>
                  )}
                </div>
