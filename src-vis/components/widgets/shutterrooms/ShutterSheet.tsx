...
export const ShutterSheet: React.FC<ShutterSheetProps> = ({
  ...
  const handlePositionEnd = () => {
    if (positionDraft !== null && device.posDp) {
      const targetRaw = device.invertPosition
        ? 100 - positionDraft
        : positionDraft;
      const targetDp = device.posDp.replace(
        /\.states\..+/,
        '.states.core:TargetClosureState'
      );
      setState(targetDp, targetRaw);
                ...
                {posQuick.map((val) => (
                  <button
                    ...
                    className={`quick-chip ${
                      Math.abs(displayPos - val) < 5 ? 'active' : ''
                    }`}
                ...
                <div className="quick-chips">
                  {slatQuick.map(([val, label]) => (
                    <button
                      key={val}
                      className={`quick-chip ${
                        Math.abs(displaySlat - val) < 5 ? 'active' : ''
                      }`}
