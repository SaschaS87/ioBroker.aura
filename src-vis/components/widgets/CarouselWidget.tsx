...
export function CarouselWidget({ config, editMode }: WidgetProps) {
    ...
    const labelAlign =
        (o.labelAlign as 'left' | 'center' | 'right' | undefined) ?? (mode === 'single' ? 'center' : 'left');
        ...
        <div className="aura-widget-row relative w-full h-full flex flex-col gap-1.5">
            ...
            {pendingItem && (
                <ConfirmOverlay
                    ...
                    onConfirm={() => {
                        dispatchAction(pendingItem.item, pendingItem.isActive);
                        setPendingItem(null);
                    }}
    ...
    return (
        ...
        >
          ...
          overflow to scroll through. */}
            {(() => {
                const itemsClass =
                    labelAlign === 'right' ? 'items-end' : labelAlign === 'center' ? 'items-center' : 'items-start';
    ...
    return (
        ...
        >
          ...
          Positioned absolutely so it doesn't contribute to layout flow. */}
            ...
            {overflow ? (
                <span className="aura-marquee-track" style={{ ['--marquee-duration' as never]: `${duration}s` }}>
                    <span>
                        {text}
                        {MARQUEE_GAP_CHARS}
                    </span>
