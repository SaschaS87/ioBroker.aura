...
import { ShutterRoomDef } from '../widgets/shutterrooms/types';
...
export const ShutterRoomsConfig: React.FC<ShutterRoomsConfigProps> = ({
  options,
  onOptionsChange,
}) => {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      deviceCount = parsed.reduce(
        (sum: number, r: ShutterRoomDef) => sum + (r.devices?.length || 0),
        0
      );
    }
  } catch (e) {
